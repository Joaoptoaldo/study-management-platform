...
public class JwtService {
    ...
    private SecretKey getSigningKey() {
        ...
        if (looksLikeHex(trimmed)) {
            // Se parecer HEX (apenas caracteres hexadecimais), decodifica como HEX
            keyBytes = hexStringToByteArray(trimmed);
        } else if (looksLikeBase64(trimmed)) {
            // Se parecer Base64, decodifica como Base64
            keyBytes = Decoders.BASE64.decode(trimmed);
        } else {
            // Caso contrário, usa UTF-8
    ...
    private boolean looksLikeBase64(String value) {
        // Base64 normalmente tem tamanho mínimo para chaves JWT e segue o pattern Base64
        return value.length() >= 32 && value.matches("^[A-Za-z0-9+/]+=*$");
    }

    private boolean looksLikeHex(String value) {
        // HEX válido normalmente tem tamanho par, pelo menos 64 caracteres e só caracteres hex
        if (value.length() % 2 != 0 || value.length() < 64) return false;
