...
import com.studyplatform.session.StudySessionService;
...
public class StudySessionController {
    ...
    @Operation(summary = "Listar sessões", description = "Retorna todas as sessões do usuário autenticado paginadas")
    @GetMapping
    public ResponseEntity<Page<StudySessionResponseDTO>> findAll(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        return ResponseEntity.ok(studySessionService.findAll(page, size));
    }
