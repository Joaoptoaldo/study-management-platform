...
public class GoalController {
    ...
    @Operation(summary = "Listar metas", description = "Retorna todas as metas do usuário autenticado paginadas")
    @GetMapping
    public ResponseEntity<Page<GoalResponseDTO>> findAll(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        return ResponseEntity.ok(goalService.findAll(page, size));
    }
